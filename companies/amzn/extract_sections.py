"""Extract key sections from Amazon 10-K HTML filings."""
import re, os
...
# Third element is the minimum plausible section length. Amazon's Item 3 is a
# one-line pointer to the contingencies note, so Item 3 and Item 4 sit ~157
# characters apart; a 500-char floor rejects the real Item 4 and the section
# runs away to the 100,000-char fallback.
SECTIONS = {
...
def at_line_start(text, pos):
    ...
    -- e.g. Amazon's Business section says 'Item 1A of Part I - "Risk Factors."'
    and its risk-factor preamble says 'Item 7 of Part II, "Management's
    Discussion..."'. Selecting either as a boundary swallows whole Items."""
...
for year in YEARS:
    ...
    for section_name, (start_pat, end_pat, min_len) in SECTIONS.items():
        ...
        if start is not None:
            ...
            section_text = ""
        with open(os.path.join(OUT_DIR, f"fy{year}_{section_name}.txt"), "w", encoding="utf-8") as f:
