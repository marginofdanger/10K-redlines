...
SECTIONS = {
    ...
    # Must match the real heading ("ITEM 3.  LEGAL PROCEEDINGS.") and NOT the
    # risk-factor cross-reference ('Item 3. "Legal Proceedings"'), whose curly
    # quote before "Legal" defeats the \s* here. The looser r"item\s*3[\.\s]"
    # latched onto that cross-reference in 6 of 9 years, over-capturing ~40-100KB
    # of Risk Factors text into the legal_proceedings extract.
    # i?tem: in FY2019 the heading's leading "I" sits in its own HTML tag, so the
    # cleaned text reads "TEM 3.    LEGAL PROCEEDINGS".
    "legal_proceedings": (r"i?tem\s*3\.\s*legal\s+proceedings", r"item\s*4[\.\s]", 100),
    # The negative lookahead rejects the table-of-contents entry, which is followed by a
    # bare page-number line; the real heading is followed by prose. find_section_boundaries'
    # own TOC heuristic does NOT catch this, because the TOC's sub-entries push the next
    # "Item" 278 chars out, clearing its 200-char threshold. Left unfixed, the extract ran
    # from the TOC to the real Item 7A, swallowing Items 1, 1A, 1B, 1C and 2-6
    # (~207KB instead of ~41KB). i?tem: in FY2018/FY2019 the leading "I" of both the
    # Item 7 and Item 7A headings sits in its own tag, rendering as "TEM 7." / "TEM 7A.".
    "mda": (r"i?tem\s*7\.\s*management[^\n]*\n(?!\s*\d+\s*\n)", r"i?tem\s*7a[\.\s]", 500),
